...
BASE_URL = "https://mundial-app-prode-frontend.vercel.app"
...
def create(body: CreateGroupRequest, user: dict = Depends(get_current_user)):
    if not body.name.strip():
        raise HTTPException(status_code=400, detail="El nombre es requerido.")
    group = create_group(body.name, body.description or "", user["id"])
...
def list_members(group_id: int, user: dict = Depends(get_current_user)):
...
def join(invite_token: str, user: dict = Depends(get_current_user)):
    ok, msg, group = join_group_by_token(invite_token, user["id"])
...
def regenerate(group_id: int, user: dict = Depends(get_current_user)):
    ok, msg = regenerate_invite_token(group_id, user["id"])
    if not ok:
